...
define method LAddRow                        
 ...
 => (result :: <integer>)
  call-out("LAddRow", int:, int: count, int: rowNum, ptr: lHandle);
end method LAddRow;

define method LDelRow                        
    (count :: <integer>, rowNum :: <integer>, lHandle :: <ListRef>)                           
 => (result :: <integer>)
  call-out("LDelRow", int:, int: count, int: rowNum, ptr: lHandle);
end method LDelRow;

define method LGetSelect
    (next :: <boolean>, theCell :: <Point*>, lHandle :: <ListRef>)
 => ()
  call-out("lgetselect", void:, int: if(next) 1 else 0 end, ptr: theCell, ptr: lHandle);
end method LGetSelect;    

define method LSize                       
    (listWidth :: <integer>, listHeight :: <integer>, lHandle :: <ListRef>)                           
 => ()
  call-out("LSize", void:, int: listWidth, int: listHeight, ptr: lHandle);
end method LSize;                 

define method LScroll                      
    (cols :: <integer>, rows :: <integer>, lHandle :: <ListRef>)                           
 => ()
  call-out("LSize", void:, int: cols, int: rows, ptr: lHandle);
end method LScroll;
...
define method LGetCell
 ...
 => (dataLen :: <integer>)
  ...
  call-out("lgetcell", void:, ptr: dataPtr.raw-value, ptr: theCell, ptr: lHandle);
...
define method LSetCell
    (dataPtr, dataLen :: <integer>, theCell :: <Point*>, lHandle :: <ListRef>)
 => ()
  call-out("lSetcell", void:, ptr: dataPtr.raw-value, int: dataLen, ptr: theCell, ptr: lHandle);
end method LSetCell;    

define method LSetSelect
    (next :: <boolean>, theCell :: <Point*>, lHandle :: <ListRef>)
 => ()
  call-out("lsetselect", void:, int: if(next) 1 else 0 end, ptr: theCell, ptr: lHandle);
end method LSetSelect;                                     
                                 
define method SetListFlags                      
    (lHandle :: <ListRef>, flags :: <integer>)                           
 => ()
  call-out("SetListFlags", void:, ptr: lHandle, int: flags);
end method SetListFlags;
                                 
define method SetListSelectionFlags                      
    (lHandle :: <ListRef>, flags :: <integer>)                           
 => ()
  call-out("SetListSelectionFlags", void:, ptr: lHandle, int: flags);
