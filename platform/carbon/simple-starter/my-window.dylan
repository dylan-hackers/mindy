...
=> ()
    ...
    if( control ~= $NULL )
        HandleControlClick( control, localPoint, event.event-modifiers, as( <ControlActionUPP>, $NULL ) );
...
=> ()
    ...
    if( charCode = '\t' )
        ...
        if( control ~= $NULL )
            HandleControlKey( control, as( <SInt16>, keyCode), as( <SInt16>, charCode), event.event-modifiers );
        end if;
    end if;
    values();
