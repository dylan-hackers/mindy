...
static CWResult	Compile(CWPluginContext context)
{
	...
	MindySettings settings = { kMindySettingsVersion, false, false, "\pm.out" };
	...
	char* outputName = (char*) gOutputFile.name + 1;
	outputName[gOutputFile.name[0]] = '\0';
	char* dot = strchr(outputName, '.');
	if (dot == NULL)
		return cwErrRequestFailed;
	strcpy(dot + 1, "dbc");
	gOutputFile.name[0] = strlen(outputName);
	gOutputPath = full_path_to(gOutputFile);
	
	int argc = 4;
	char* argv[] = { "mindy", gSourcePath, "-o", gOutputPath, NULL };
