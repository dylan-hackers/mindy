...
define method decode-escape-character
 ...
 => result :: <integer>;
  let contents = source-location.source.contents;
  if (contents[start] ~= '<')
    let char = select (contents[start])
                 'a' => '\a';
                 'b' => '\b';
                 'e' => '\e';
                 'f' => '\f';
...
define method parse-integer-literal
  ...
  make(<literal-token>,
       source-location: source-location,
       kind: $literal-token,
       literal: make(<literal-integer>, value: int));
...
define class <state> (<object>)
  ...
  // complex tokens.
  slot result :: type-union(singleton(#f), <symbol>, <integer>, <function>),
...
define method state
    (name :: <symbol>,
     result :: type-union(singleton(#f), <symbol>, <integer>, <function>),
     #rest transitions)
  //
