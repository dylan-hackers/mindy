...
######################################################################
##
##  $Header: /home/housel/work/rcs/gd/src/mindy/libraries/dylan/deque.dylan,v 1.1 1994/03/24 21:49:54 wlott Exp $
##
...
###
# define method print-object(object :: <deque-element>,
# 			   stream :: <output-stream>)
define method print-object(object :: <deque-element>, stream)
  format(stream("{Deque-element ~S}", deque-element-data(object)));
...
define method initialize(deque :: <deque>,
			 #key data: data, size: size (0),
			      fill: fill ($deque-fill-default$))
  deque-head(deque) := #f;
  deque-tail(deque) := #f;
  if (data)
    for (element in data)
...
### print-object -- interface
###
# define method print-object(deque :: <deque>, stream :: <output-stream>)
define method print-object(deque :: <deque>, stream)
  format(stream,"{Deque");
...
### until the size is N.
###
define method size-setter(n :: <integer>, deque :: <deque>) :: <integer>;
  let s = size(deque);
  if (n < s)
...
define method remove(deque :: <deque>, value,
		     #key test: test (`==`), count: count) :: <deque>;
...
###
define method last(deque :: <deque>, #key default: defalut (no_default))
