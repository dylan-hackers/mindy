...
enum constituent_kind {
    constituent_DEFCONST,
    constituent_DEFVAR,
    constituent_DEFMETHOD,
    constituent_DEFDOMAIN,
    constituent_DEFGENERIC,
    constituent_DEFCLASS,
    constituent_EXPR,
    constituent_LOCAL,
    constituent_HANDLER,
    constituent_LET,
    constituent_TOPLEVELFORM,
    constituent_ERROR,
    constituent_DEFMODULE,
    constituent_DEFLIBRARY,
    constituent_Kinds
};
...
extern struct body *add_constituent(struct body *body,
                                    struct constituent *constituent);
extern struct body *make_expr_body(struct expr *expr);
extern struct constituent *make_define_constant(int line,
                                                struct bindings *bindings);
extern struct constituent *make_define_method(flags_t flags,
                                              struct method *method);
extern struct constituent *make_define_function(flags_t flags,
                                                struct method *method);
extern struct constituent *make_define_variable(int line,
                                                struct bindings *bindings);
...
extern struct expr *make_binop_series_expr(struct expr *operand,
                                           struct binop_series *series);
extern struct binop_series *make_binop_series(void);
extern struct binop_series *add_binop(struct binop_series *series,
                                      struct binop *op,
                                      struct expr *operand);
...
extern struct expr *make_aref_or_element(int line, struct expr *expr,
                                         struct arglist *args);
extern struct expr *make_function_call(struct expr *expr,
                                       struct arglist *args);
...
extern struct argument *make_keyword_argument(struct token *keyword,
                                              struct expr *expr);
extern struct plist *make_property_list(void);
extern struct plist *add_property(struct plist *plist, struct token *keyword,
                                  struct expr *expr);
...
extern struct return_type_list *add_return_type(struct return_type_list *l,
                                                struct expr *type);
extern struct return_type_list *set_return_type_rest_type(struct return_type_list *l,
                                                          struct expr *type);
...
extern struct literal *concat_string_token(struct literal *old_literal,
                                           struct token *token);
...
extern struct exception_clauses *add_exception_clause(struct exception_clauses *clauses,
                                                      struct exception_clause *clause);
extern struct exception_clause *make_exception_clause(struct expr *type,
                                                      struct id *condition,
                                                      struct plist *plist,
                                                      struct body *body);
extern struct condition_body *push_condition_clause(struct condition_clause *clause,
                                                    struct condition_body *cond_body);
extern struct condition_clause *make_otherwise_condition_clause(struct body *body);
extern struct incomplete_condition_body *make_incomplete_condition_clauses(struct constituent *constituent,
                                                                           struct condition_body *rest);
extern struct incomplete_condition_body *push_condition_constituent(struct constituent *constituent,
                                                                    struct incomplete_condition_body *body);
extern struct condition_body *complete_condition_clauses(struct condition_clause *clause,
                                                         struct incomplete_condition_body *body);
extern struct condition_clause *make_condition_clause(struct constituent *constituent);
extern struct condition_clause *push_condition(struct expr *cond,
                                               struct condition_clause *clause);
extern struct for_clause *make_equal_then_for_clause(struct param_list *vars,
                                                     struct expr *equal,
                                                     struct expr *then);
extern struct for_clause *make_in_for_clause(struct param *var,
                                             struct param *keyed_by,
                                             struct expr *collection,
                                             struct param *protocol);
extern struct for_clause *make_from_for_clause(struct param *var,
                                               struct expr *from,
...
extern struct constituent *make_class_definition(struct id *name,
                                                 struct superclass_list *supers,
                                                 struct class_guts *guts);
extern struct constituent *set_class_flags(flags_t flags,
                                           struct constituent *defclass);
extern struct superclass_list *make_superclass_list(void);
extern struct superclass_list *add_superclass(struct superclass_list *list,
                                              struct expr *expr);
extern struct class_guts *make_class_guts(void);
extern struct slot_spec *make_slot_spec(int line, flags_t flags,
                                        enum slot_allocation alloc,
                                        struct id *name, struct expr *type,
                                        struct expr *init_expr,
                                        struct plist *plist);
extern struct class_guts *
add_slot_spec(struct class_guts *guts, struct slot_spec *spec);
extern struct initarg_spec *make_initarg_spec(bool required,
                                              struct token *keyword,
                                              struct plist *plist);
extern struct class_guts *add_initarg_spec(struct class_guts *guts,
                                           struct initarg_spec *spec);
extern struct inherited_spec *make_inherited_spec(int line, struct id *name,
                                                  struct expr *init_expr,
                                                  struct plist *plist);
extern struct class_guts *add_inherited_spec(struct class_guts *guts,
                                             struct inherited_spec *spec);
extern struct constituent *make_sealed_domain(struct id *name,
                                              struct arglist *types);
extern struct constituent *set_sealed_domain_flags(flags_t flags, struct constituent *sealed_domain);
extern struct constituent *make_define_generic(struct id *name, struct param_list *params,
                                               struct gf_suffix *suffix);
extern struct constituent *set_generic_flags(flags_t flags, struct constituent *defgeneric);
extern struct gf_suffix *make_gf_suffix(struct return_type_list *rettypes,
                                        struct plist *plist);
extern struct param_list *push_keyword_param(struct keyword_param *param,
                                             struct param_list *list);
...
extern struct keyword_param *make_keyword_param(struct token *keyword,
                                                struct id *sym,
                                                struct expr *type, struct expr *def);
extern struct method *set_method_source(struct token *source,
                                        struct method *method);
...
extern struct param_list *set_next_param(struct param_list *list,
                                         struct id *var);
...
extern struct defnamespace_constituent *set_namespace_name(struct defnamespace_constituent *namespace,
                                                           struct token *name);
extern struct defnamespace_constituent *add_use_clause(struct defnamespace_constituent *namespace,
                                                       struct use_clause *clause);
extern struct defnamespace_constituent *add_exports(struct defnamespace_constituent *namespace,
                                                    struct variable_names *vars);
extern struct defnamespace_constituent *add_creates(struct defnamespace_constituent *namespace,
                                                    struct variable_names *vars);
extern struct use_clause *make_use_clause(struct token *symbol,
                                          struct use_options *options);
extern struct use_options *make_use_options(void);
extern struct use_options *add_use_option(struct use_options *options,
...
extern struct variable_names *add_variable_name(struct variable_names *names,
                                                struct token *token);
extern struct renamings *make_renamings(void);
extern struct renamings *add_renaming(struct renamings *names,
                                      struct token *from, struct token *to);
extern struct import_option *make_import_option(void);
extern struct import_option *add_import(struct import_option *opt,
                                        struct token *from, struct token *to);
