...
// Work around an apparent bug in streams.
//
//define inline method as (type == <byte-character>, object :: <integer>)
...
define method check-seg-fault (str :: <string>) => (result :: <boolean>);
  if (match-strings(str, "Program received signal SIGSEGV", 0))
    do-gdb-command("set dylan_no_applicable_methods_error_main(orig_sp)\n");
...
define method select-dylan-variable
    (dylan-name :: <string>) => (result :: false-or(<string>));
  let (#rest vars) = find-dylan-variable(dylan-name);
  select (vars.size)
    0 => #f;
...
end method select-dylan-variable;
...
define method generic-function-main-entries
...
end method generic-function-main-entries;

define method set-generic-breakpoints (fun :: <string>) => ();
  let (#rest entries) = generic-function-main-entries(fun);
