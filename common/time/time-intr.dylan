...
copyright: See below.
rcs-header: $Header: /scm/cvs/src/common/time/Attic/time-intr.dylan,v 1.5 2000/11/07 17:47:48 robmyers Exp $
...
#if (compiled-for-cygnus | compiled-for-darwin)	
	  // cygnus does have <sys/time.h>, but it doesn't have
	  // long daylight in that header file
	  method() error("This system does not implement daylight") end;
#else
	  find-c-pointer("daylight");
#endif
...
define sealed method daylight () => (result :: <integer>);
#if (compiled-for-cygnus | compiled-for-darwin)
  error("This system does not implement daylight");
  0;
#else
